...
"""Various network "heads" for predicting boundarys in Mask R-CNN.

The design is as follows:

... -> RoI ----\
                -> RoIFeatureXform -> boundary head -> boundary output -> loss
... -> Feature /
       Map

The boundary head produces a feature representation of the RoI for the purpose
of boundary prediction. The boundary output module converts the feature representation
into real-valued (soft) boundarys.
"""
...
def boundary_fcn_head_v1up4convs(model, blob_in, dim_in, spatial_scale):
    """v1up design: 4 * (conv 3x3), convT 2x2."""
    return boundary_rcnn_fcn_head_v1upXconvs(
        model, blob_in, dim_in, spatial_scale, 4
    )


def boundary_rcnn_fcn_head_v1up(model, blob_in, dim_in, spatial_scale):
...
def boundary_rcnn_fcn_head_v0upshare(model, blob_in, dim_in, spatial_scale):
    """Use a ResNet "conv5" / "stage5" head for boundary prediction. Weights and
    computation are shared with the conv5 box head. Computation can only be
    shared during training, since inference is cascaded.

    v0upshare design: conv5, convT 2x2.
    """
    # Since box and boundary head are shared, these must match
    assert cfg.BOUNDARY.ROI_XFORM_RESOLUTION == cfg.FAST_RCNN.ROI_XFORM_RESOLUTION

    if model.train:  # share computation with bbox head at training time
        dim_conv5 = 2048
        blob_conv5 = model.net.SampleAs(
            ['res5_2_sum', 'roi_has_boundary_int32'],
            ['_[boundary]_res5_2_sum_sliced']
        )
    else:  # re-compute at test time
        blob_conv5, dim_conv5 = add_ResNet_roi_conv5_head_for_boundarys(
            model,
            blob_in,
            dim_in,
            spatial_scale
    ...
    dim_reduced = cfg.BOUNDARY.DIM_REDUCED

    blob_boundary = model.ConvTranspose(
        blob_conv5,
        'conv5_boundary',
        dim_conv5,
        dim_reduced,
        ...
        pad=0,
        stride=2,
        weight_init=(cfg.BOUNDARY.CONV_INIT, {'std': 0.001}),  # std only for gauss
        bias_init=const_fill(0.0)
    )
    model.Relu('conv5_boundary', 'conv5_boundary')

    return blob_boundary, dim_reduced


def boundary_rcnn_fcn_head_v0up(model, blob_in, dim_in, spatial_scale):
...
def add_ResNet_roi_conv5_head_for_boundarys(model, blob_in, dim_in, spatial_scale):
    """Add a ResNet "conv5" / "stage5" head for predicting boundarys."""
    model.RoIFeatureTransform(
        blob_in,
        blob_out='_[boundary]_pool5',
        blob_rois='boundary_rois',
        method=cfg.BOUNDARY.ROI_XFORM_METHOD,
        resolution=cfg.BOUNDARY.ROI_XFORM_RESOLUTION,
        sampling_ratio=cfg.BOUNDARY.ROI_XFORM_SAMPLING_RATIO,
    ...
    dilation = cfg.BOUNDARY.DILATION
    stride_init = int(cfg.BOUNDARY.ROI_XFORM_RESOLUTION / 7)  # by default: 2
