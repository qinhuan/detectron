...
def add_boundary_blobs(blobs, sampled_boxes, roidb, im_scale, batch_idx):
    ...
    if cfg.BOUNDARY.CLS_SPECIFIC_MASK:
        boundarys = _expand_to_class_specific_boundary_targets(boundarys, boundary_class_labels)
...
def _expand_to_class_specific_boundary_targets(boundarys, boundary_class_labels):
...
def get_boundary(mask):
    """Get boundary from mask gt.
    """
    h, w = mask.shape
    boundary = np.zeros((h, w), dtype=np.int32)
    for y in range(h - 1):
        for x in range(w - 1):
